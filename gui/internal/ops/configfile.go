...
// SaveSchedule 只替换 config.json 的 schedule 段并落盘（读整份文档 → 改一段 → 原子写回），
// 返回写入的 schedule 供调用方随后调网关 /admin/automation/apply 热生效。
//
// 为什么由面板写文件：网关容器内 config.json 是只读挂载，网关侧 apply 只改内存；
// 面板是唯一文件写者，避免两个写者互相覆盖。
func (s *Service) SaveSchedule(schedule map[string]any) (map[string]any, error) {
	doc, _, err := s.ReadUpstreamConfig()
	if err != nil {
		return nil, err
	}
	if doc == nil {
	...
	// 合并语义：只覆盖传入的键，保留 schedule 段的其它字段
	//（如 activity_report_count —— 该字段不支持热更新，面板表单另行编辑，不能被整段替换抹掉）。
	existing, _ := doc["schedule"].(map[string]any)
	merged := make(map[string]any, len(existing)+len(schedule))
	...
	doc["schedule"] = merged
	if err := s.WriteUpstreamConfig(doc); err != nil {
		return nil, err
	}
	return merged, nil
}
