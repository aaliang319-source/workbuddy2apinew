...
export const api = {
  ...
  // 配置
  // 自动化（代理网关 /admin/automation/*）
  automationStatus: () => get<AutomationStatus>('/api/automation/status'),
  automationRuns: (limit = 20) => get<{ runs: AutomationRun[] }>(`/api/automation/runs?limit=${limit}`),
  automationRun: (id: string) => get<AutomationRun>(`/api/automation/runs/${encodeURIComponent(id)}`),
  automationTrigger: (kind: string) =>
    post<{ run: AutomationRun }>(`/api/automation/${encodeURIComponent(kind)}/run`),
  automationApply: (schedule: Record<string, unknown>) =>
    post<{ ok: boolean; status: AutomationStatus }>('/api/automation/apply', { schedule }),
  ...
  // 通知（邮件提醒）
  notifyTest: () => post<{ ok: boolean; message: string }>('/api/notify/test'),

  // 系统
  system: () => get<SystemInfo>('/api/system'),
  restart: () => post<{ ok: boolean; message: string }>('/api/system/restart'),

  // API Key 管理（代理网关 /admin/keys）
  keys: () => get<{ keys: ApiKeyEntry[] }>('/api/keys'),
  ...
  regenerateKey: (id: string) =>
    post<ApiKeyEntry>(`/api/keys/${encodeURIComponent(id)}/regenerate`),
  toggleAccount: (uid: string, disabled: boolean, reason?: string) =>
    post<{ ok: boolean; message: string; disabled: boolean }>(
      `/api/accounts/${encodeURIComponent(uid)}/toggle`,
      { disabled, reason },
    ),
}
