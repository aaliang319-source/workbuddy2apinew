...
import { buildCcSwitchLink, parseListenPort, type CcSwitchFields } from '../ccswitch'

// ─── CC Switch 一键导入 ────────────────────────────────────────────────
// 深链生成逻辑已抽取到 ../ccswitch.ts（System 页与 Keys 页共用）。

/** CcSwitchCard CC Switch 供应商一键导入卡片。 */
function CcSwitchCard() {
  const [loaded, setLoaded] = useState(false)
  const [loadError, setLoadError] = useState<string | null>(null)
  const [copied, setCopied] = useState<'claude' | 'codex' | null>(null)
  const [models, setModels] = useState<string[]>([]) // 网关 /v1/models 拉取的模型 id，供下拉选
  const [f, setF] = useState<CcSwitchFields>({
    endpoint: '',
    apiKey: '',
    model: 'cn:glm-5.3',
    haiku: 'cn:glm-5.3-flash',
    sonnet: 'cn:glm-5.3',
    opus: 'global:deep-model',
    codexModel: 'cn:glm-5.3',
  })
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // endpoint 用浏览器视角拼：面板的 gateway_url 是容器视角（host.docker.internal），
        // 宿主机浏览器打不开；用户浏览器能打开面板，同主机名的网关端口即可达。
        const endpoint = `http://${window.location.hostname}:${parseListenPort(cfg.listen)}`
        setF((prev) => ({ ...prev, endpoint, apiKey }))
        try {
          const m = await api.models()
          if (!cancelled) setModels((m.data ?? []).map((x) => x.id))
        } catch {
          // 模型列表拉取失败不阻塞：字段回退为文本输入
        }
        setLoaded(true)
      } catch (err) {
        if (!cancelled) setLoadError(err instanceof ApiError ? err.message : '读取网关配置失败')
      }
    })()
    return () => {
      cancelled = true
  ...
  const set = (k: keyof CcSwitchFields) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    setF((prev) => ({ ...prev, [k]: e.target.value }))

  // 模型字段：网关模型列表可用时用下拉选；拉取失败/为空回退文本输入（允许手填任意模型名）。
  const modelField = (k: 'model' | 'haiku' | 'sonnet' | 'opus' | 'codexModel', label: string) => (
    <div className="field" style={{ flex: '1 1 160px' }}>
      <label>{label}</label>
      {models.length > 0 ? (
        <select className="mono" value={f[k]} onChange={set(k)}>
          {(models.includes(f[k]) || f[k] === '' ? models : [f[k], ...models]).map((id) => (
        ...
        <input className="mono" value={f[k]} onChange={set(k)} />
      )}
    </div>
  )

  const copyLink = async (app: 'claude' | 'codex') => {
    try {
      await navigator.clipboard.writeText(buildCcSwitchLink(app, f))
      setCopied(app)
      setTimeout(() => setCopied(null), 2000)
    } catch {
      // 剪贴板不可用（非 HTTPS / 无权限）时让用户从预览框手动复制。
    }
  }

  const claudeLink = buildCcSwitchLink('claude', f)
  const codexLink = buildCcSwitchLink('codex', f)

  return (
    <div className="card">
      <div className="card-head">
        <h2>CC Switch 一键导入</h2>
        <span className="hint">协议直连 · 导入即用</span>
      </div>
      <p className="text-dim" style={{ marginTop: 0, fontSize: 13 }}>
        ...
        ：点击下方按钮会唤起本机 CC Switch 并弹出确认框。网关已原生支持双协议——
        Anthropic Messages（/v1/messages，Claude Code 直连）与 OpenAI Chat/Responses
        （Codex 直连），导入后无需任何协议转换组件。
      </p>
      {loadError && <Alert kind="warn">{loadError}（仍可手动填写下方字段生成链接）</Alert>}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <div className="field" style={{ flex: '1 1 260px' }}>
          <label>网关地址 endpoint</label>
          <input
            className="mono"
            value={f.endpoint}
            onChange={set('endpoint')}
            placeholder={loaded ? '' : 'http://127.0.0.1:7863'}
          />
        </div>
        <div className="field" style={{ flex: '1 1 260px' }}>
          <label>API Key</label>
          <input className="mono" value={f.apiKey} onChange={set('apiKey')} placeholder="网关 config.json 的 api_key" />
        </div>
      </div>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 4 }}>
        {modelField('model', 'Claude 默认模型')}
        {modelField('haiku', 'Haiku 档')}
        {modelField('sonnet', 'Sonnet 档')}
        {modelField('opus', 'Opus 档')}
        {modelField('codexModel', 'Codex 模型')}
      </div>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 10 }}>
        <div>
          <a className="btn btn-primary" href={claudeLink} style={{ textDecoration: 'none' }}>
            导入 Claude Code 供应商
          </a>{' '}
          <button className="btn" onClick={() => void copyLink('claude')}>
            {copied === 'claude' ? '已复制 ✓' : '复制 Claude 链接'}
          </button>
        </div>
        <div>
          <a className="btn btn-primary" href={codexLink} style={{ textDecoration: 'none' }}>
            导入 Codex 供应商
          </a>{' '}
          <button className="btn" onClick={() => void copyLink('codex')}>
            {copied === 'codex' ? '已复制 ✓' : '复制 Codex 链接'}
          </button>
        </div>
      </div>
      <div className="muted-box mono" style={{ marginTop: 10, wordBreak: 'break-all', fontSize: 12 }}>
        {claudeLink}
      </div>
      <div className="desc" style={{ marginTop: 10 }}>
        提示：链接包含<strong>明文 API Key</strong>，仅限本机/可信渠道使用，勿粘贴到公开场合。
        模型名带 <span className="mono">cn:</span>/<span className="mono">global:</span> 前缀时按域路由账号池；
        Claude 三档映射决定 Claude Code 的主模型/快速小模型，其余未知模型名走网关
        <span className="mono"> anthropic.default_model</span> 兜底（缺省 cn:auto）。
        在 CC Switch 中切换到该供应商后，Claude Code / Codex 无需再改任何配置。
      </div>
    </div>
  )
}
