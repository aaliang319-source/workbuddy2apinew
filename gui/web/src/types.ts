...
export interface RequestRecord {
  ...
  cache_miss_tokens?: number
  credit?: number
  /** 失败原因（权威分类，如 hard_credit(402)）；成功时缺失 */
  err?: string
  /** 协议来源：openai（/v1/chat/completions）/ anthropic（/v1/messages）；旧记录缺失 */
  proto?: string
  /** 命中的业务 Key 名（多 Key 模式）；旧记录缺失 */
  key_name?: string
  /** 轮转尝试次数；>1 表示换过号（限流/失效排查）；旧记录缺失 */
  tries?: number
...
// ─── API Key 管理（网关 /admin/keys 镜像）──────────────────────────────

/** KeyAssociation Key↔账号关联（priority 越大越优先，分层调度先耗尽最高层）。 */
...
// ── 自动化（/api/automation/*，数据源为网关 /admin/automation/*）────────────

/** 单条执行结果（账号类任务一账号一条；脚本类任务一条，output 在 run 上）。 */
export interface AutomationItem {
  uid?: string
  nickname?: string
  ok: boolean
  status: 'ok' | 'already' | 'fail' | 'skipped' | string
  message?: string
  reward?: number
  credits?: number | null
}
...
/** 单个任务类型的实时状态。 */
export interface AutomationKindStatus {
  kind: string
  enabled: boolean
  hours: number[] | null
