...
func TestTemplateContainsKeyFields(t *testing.T) {
	...
	n.OnPoolEvent(pool.NoticeEvent{
		Kind: "cooling", UID: "9fa5b964-2c60-42f6-83cd-4833fee9c05f", Nickname: "Alvin",
		Realm: "global", Reason: "429 rate limit", Credits: 2364, Expiring: 100,
	})
	got := waitMail(t, f, 1)
	body := got[0].Body
	for _, want := range []string{"账号路由切换", "Alvin", "9fa5b964", "国际 global", "2364", "429 rate limit", "alive1"} {
