...
func TestApplyErrorPolicyAccountFaultSplit(t *testing.T) {
	...
	t.Run("11140 request illegal disables", func(t *testing.T) {
		...
		h.applyErrorPolicy("u1", upstream.ErrAccountFault, `{"error":{"data":{"code":11140,"msg":"request illegal"}}}`, "glm-5.2", 0)
		st, _ := p.Status("u1")
		if !st.Disabled {
	...
	t.Run("14017 trial stays soft cool", func(t *testing.T) {
		...
		h.applyErrorPolicy("u1", upstream.ErrAccountFault, `{"error":{"data":{"code":14017,"msg":"The trial version is not yet activated"}}}`, "glm-5.2", 0)
		st, _ := p.Status("u1")
		if st.Disabled {
...
func TestApplyErrorPolicySoftRateNoDoubleWhenCooling(t *testing.T) {
	...
	h.applyErrorPolicy("u1", upstream.ErrSoftRate, "", "", 0)
	st, _ := p.Status("u1")
	if !st.Cooling || st.CoolKind != "soft_rate" {
...
func TestApplyErrorPolicySoftRateResetTime11140(t *testing.T) {
	...
	h.applyErrorPolicy("u1", upstream.ErrSoftRate, body, "glm-5.3", 0)
	st, ok := p.Status("u1")
	if !ok {
		t.Fatal("u1 missing")
	}
...
func TestApplyErrorPolicyNotFoundUsesFixedBase(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		h.applyErrorPolicy("u1", upstream.ErrNotFound, "", "", 0)
		st, _ := p.Status("u1")
		if !st.Cooling || st.CoolKind != "soft_rate" {
